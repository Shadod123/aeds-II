/*----------------------------------------------------------------
Atividade: TP03Q15 - Pilha Flexivel em C
------------------------------------------------------------------
...
#include<stdbool.h>
    ...
    */ 
    ...
    /**
     * Mostra os elementos, um em cada linha, comecando do topo.
     */
    void mostrar() {
        Celula* i;
     ...
     */
    void readInputPart2 (Celula* topo){
        int i = 0;
        int n = 0;
        int qtdOps = 0;
        char blankSpace;
        ...
        for(int i = 0; i < qtdOps; i++){
            ...
            //Comando = I
            if (comando[0] == 'I'){
                strncat(path,complemento,sizeof(complemento));
                item = read(path);
                clone = clonar(item);
                inserir(clone);
            }
