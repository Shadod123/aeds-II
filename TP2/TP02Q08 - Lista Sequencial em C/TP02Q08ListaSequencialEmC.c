/*----------------------------------------------------------------
Atividade: TP02Q08 - Lista Sequencial em C
------------------------------------------------------------------
...
#include<stdbool.h>
    ...
    */ 
    ...
    /**
     * Faz o tratamento da segunda parte da entrada (operacoes de insercao
     * e remocao na lista preenchida na primeira parte).
     * @param lista - lista onde as insercoes e remocoes serao feitas.
     */
    void readInputPart2 (Lista* lista){
        int i = 0;
        int n = 0;
        int qtdOps = 0;
        char blankSpace;
        ...
        for(int i = 0; i < qtdOps; i++){
            //Garante que os vetores fiquem limpos/sem lixo para cada ciclo 
            memset(comando, 0, 100);
            memset(complemento1, 0, 100);
            memset(complemento2, 0, 100);
            memset(path, 0, 200);
            ...
            if (!(comando[0] == 'R' && comando[1] == 'I' || 
                  comando[0] == 'R' && comando[1] == 'F')){
                //O comando I* possui dois complementos      
                if (comando[0] == 'I' && comando[1] == '*'){
                    scanf("%s", complemento1);
                    scanf("%c", &blankSpace);
                    fgets(complemento2, 100, stdin);
                    setbuf(stdin, NULL);
                    if(complemento2[strlen(complemento2)-1] == '\n'){ complemento2[strlen(complemento2)-1] = '\0'; }
                }
                //Os demais comandos requerem somente um complemento
                else{
                    scanf("%c", &blankSpace);
                    fgets(complemento1, 100, stdin);
                    setbuf(stdin, NULL);
                    if(complemento1[strlen(complemento1)-1] == '\n'){ complemento1[strlen(complemento1)-1] = '\0'; }
                }             
            }

            //Comando = II
            if (comando[0] == 'I' && comando[1] == 'I'){
                strncat(path,complemento1,sizeof(complemento1));
                item = read(path);
                clone = clonar(item);
                inserirInicio(lista,clone);
            }
            //Comando = I*
            else if (comando[0] == 'I' && comando[1] == '*'){
                strncat(path,complemento2,sizeof(complemento2));
                item = read(path);
                clone = clonar(item);
                inserir(lista, clone, atoi(complemento1)); 
            }
            //Comando = IF
                ...
                item = read(path);
                clone = clonar(item);
                inserirFim(lista,clone); 
            }
            //Comando = RI
            else if (comando[0] == 'R' && comando[1] == 'I'){
                deleted = removerInicio(lista);
                printf("(R) %s\n", deleted.name);
            }
            //Comando = R*
            else if (comando[0] == 'R' && comando[1] == '*'){
                deleted = remover(lista,atoi(complemento1));
                printf("(R) %s\n", deleted.name);
            }
            //Comando = RF
            else if (comando[0] == 'R' && comando[1] == 'F'){
                deleted = removerFim(lista);
                printf("(R) %s\n", deleted.name); 
            }
        }
