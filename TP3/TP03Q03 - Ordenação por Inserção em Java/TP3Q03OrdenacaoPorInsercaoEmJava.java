/*----------------------------------------------------------------
Atividade: TP03Q03 - Ordenacao por Insercao em Java
------------------------------------------------------------------
...
class Lista {
    ...
    */
	 ...
	 */
    public void insertionSortOgLanguage() {
        for (int i = 1; i < n; i++) {
            Serie tmp = series[i];
            int j = i - 1;

            while ((j >= 0) && stringTrim(series[j].getOriginalLanguage()).compareTo(stringTrim(tmp.getOriginalLanguage())) > 0) {
                comp_num++;
    ...
    public void selectionSortName(){
        ...
        for (int i = 0; i < (n - 1); i++) {
            ...
            if (stringTrim(series[i].getOriginalLanguage()).equals(stringTrim(series[i+1].getOriginalLanguage()))) {
                for (int j = (i + 1); j < n && stringTrim(series[menor].getOriginalLanguage()).equals(stringTrim(series[j].getOriginalLanguage())); j++) {
...
public class TP3Q03OrdenacaoPorInsercaoEmJava {
