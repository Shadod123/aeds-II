/*----------------------------------------------------------------
Atividade: TP03Q10 - Radixsort em Java
------------------------------------------------------------------
...
class Lista {
    ...
    */
     ...
     */
    public int getMaior() {
        int maior = series[0].getEpisodes() * 1000 + series[0].getSeasons();

        for (int i = 0; i < n; i++) {
            int crazyValue = series[i].getEpisodes() * 1000 + series[i].getSeasons();
    ...
    public void countingSort(int exp) {
        ...
        int[] count = new int[10]; //new int[10]
        Lista ordenada = new Lista(100);
        ...
        for (i = 0; i < n; i++){
            count[ ( (series[i].getEpisodes() * 1000 + series[i].getSeasons()) / exp ) % 10 ]++;
        }
        ...
        //Ordenando                            
        for(i = n-1; i >= 0; i--){
            ordenada.series[ count[ ( (series[i].getEpisodes() * 1000 + series[i].getSeasons() ) / exp) % 10 ]-1 ] = series[i]; 
            mov_num++;
            count[ ( (series[i].getEpisodes() * 1000 + series[i].getSeasons()) / exp ) % 10 ]--;
        }
        
        //Copiando para a lista original
        for(i = 0; i < n; series[i] = ordenada.series[i], i++){
            mov_num++;
        }
    }
	 ...
	 */
	public void radixSortCrazyValue(){
		int m = getMaior();
     ...
     * anteriormente sobre o valor doidao. Reordena, em ordem
    ...
    public void selectionSortName(){
        ...
        for (int i = 0; i < (n - 1); i++) {
            int menor = i;
            int crazyValue1 = series[i].getEpisodes() * 1000 + series[i].getSeasons();
            int crazyValue2 = series[i+1].getEpisodes() * 1000 + series[i+1].getSeasons();
        
        
            if (crazyValue1 == crazyValue2) {
                for (int j = (i + 1); j < n && ( series[menor].getEpisodes() * 1000 + series[menor].getSeasons() ) == ( series[j].getEpisodes() * 1000 + series[j].getSeasons() ); j++) {
...
public class TP3Q10RadixsortEmJava {
