/*----------------------------------------------------------------
Atividade: TP03Q07 - CountingSort em C
------------------------------------------------------------------
...
#include<time.h>
    ...
    */
    ...
    int getMaior(Lista* lista) {
        int maior = lista->series[0].seasons;

        for (int i = 0; i < lista->n; i++) {
            if(maior < lista->series[i].seasons){
                maior = lista->series[i].seasons;
            } comp_num++;
        }
    ...
    void countingSortSeasons(Lista* lista) {
        //Array para contar o numero de ocorrencias de cada elemento
        int tamCount = getMaior(lista) + 1;
        int count[tamCount];
        Lista ordenada = new_Lista();

        //Inicializar cada posicao do array de contagem 
        for (int i = 0; i < tamCount; count[i] = 0, i++);

        //Agora, o count[i] contem o numero de elemento iguais a i
        for (int i = 0; i < lista->n; count[lista->series[i].seasons]++, i++);

        //Agora, o count[i] contem o numero de elemento menores ou iguais a i
        for(int i = 1; i < tamCount; count[i] += count[i-1], i++);

        //Ordenando
        for(int i = lista->n-1; i >= 0; ordenada.series[count[lista->series[i].seasons]-1] = lista->series[i], count[lista->series[i].seasons]--, i--){
            mov_num++;
        }

        //Copiando para a lista original
        for(int i = 0; i < lista->n; lista->series[i] = ordenada.series[i], i++){
            mov_num++;
        }
    }    
     ...
     * anteriormente sobre o atributo seasons. Reordena, 
     * em ordem alfabetica, os items da lista que compartilham do 
     * mesmo valor para o atributo seasons.
     */
    void selectionSortName(Lista* lista){
        ...
        for (int i = 0; i < (lista->n - 1); i++) {
            ...
            if ( lista->series[i].seasons == lista->series[i+1].seasons ) {
                for (int j = (i + 1); j < lista->n && lista->series[menor].seasons == lista->series[j].seasons; j++) {
