/*----------------------------------------------------------------
Atividade: TP03Q01 - Ordenacao por Selecao Recursiva em C
------------------------------------------------------------------
...
#include<time.h>
    ...
    */
    ...
    /**
     * Metodo auxiliar da funcao recursiveSelectionSortCountry.
     * 
     * @param lista - lista a ter seus elementos exibidos.
     * @param menor - atual menor elemento encontrado na lista.
     * @param j - variavel auxiliar no processo recursivo (posicao
     *            dos elementos da lista a serem comparados com o
     *            menor).
     * @return menor - menor elemento encontrado apos percorrer a
     *                 a parte ainda nao ordenada da lista.
     */
    int recursiveSelectionSortCountryAuxFunc(Lista* lista, int menor, int j){
        if (j < lista->n) {
            menor = recursiveSelectionSortCountryAuxFunc(lista, menor, j+1);

            if (strcmp( stringTrim2(lista->series[menor].country) , stringTrim2(lista->series[j].country) ) > 0){
               menor = j;
            } comp_num++;  
        } 
    ...
    /**
     * Ordena, em ordem alfabetica e recursivamente, os 
     * elementos da lista, com base no atributo country 
     * e por meio do algoritmo de ordenacao por selecao. 
     * 
     * @param lista - lista a ter seus elementos ordenados.
     * @param i - variavel auxiliar no processo recursivo (a principio, e' 
     *            a posicao inicial [0] da lista).
     */
    void recursiveSelectionSortCountry(Lista* lista, int i){
        ...
        if (i < (lista->n - 1)){
            ...
            menor = recursiveSelectionSortCountryAuxFunc(lista, menor, j);
            ...
            recursiveSelectionSortCountry(lista,i+1);
        }  
    ...
    /**
     * Metodo auxiliar da funcao recursiveSelectionSortName.
     * 
     * @param lista - lista a ter seus elementos exibidos.
     * @param menor - atual menor elemento encontrado na lista.
     * @param j - variavel auxiliar no processo recursivo (posicao
     ...
     *                 a parte ainda nao ordenada da lista.
     */
    int recursiveSelectionSortNameAuxFunc(Lista* lista, int menor, int j){
        if (j < lista->n && strcmp( stringTrim2(lista->series[menor].country) , stringTrim2(lista->series[j].country) ) == 0) {
            menor = recursiveSelectionSortNameAuxFunc(lista, menor, j+1);
     ...
     * alfabetica e recursivamente, os items da lista que compartilham 
     * do mesmo valor para o atributo country.
     * 
     * @param lista - lista a ter seus elementos ordenados.
     * @param i - variavel auxiliar no processo recursivo (a principio, e' 
     *            a posicao inicial [0] da lista).
     */
    void recursiveSelectionSortName(Lista* lista, int i){
        ...
        if (i < (lista->n - 1)){
            ...
            if ( strcmp( stringTrim2(lista->series[i].country) , stringTrim2(lista->series[i+1].country) ) == 0 ) {
                menor = recursiveSelectionSortNameAuxFunc(lista, menor, j);
            } comp_num++;
            ...
            lista->series[i] = tmp;
            mov_num = mov_num + 3;
            recursiveSelectionSortName(lista,i+1);
        } 
    ...
    int main ( ){
        ...
        start = clock();
        recursiveSelectionSortCountry(&lista,0);
        recursiveSelectionSortName(&lista,0);
        end = clock();

        mostrar(&lista);
        free(lista.series);

        fp = fopen("matricula_selecaoRecursiva.txt","w");
